...
interface ApiKeyModalProps {
  ...
  onSave: (keys: { gemini?: string; pexels?: string }) => void;
}
...
export const ApiKeyModal: React.FC<ApiKeyModalProps> = ({ isOpen, missingKeys, onSave }) => {
  ...
  const handleSave = () => {
    onSave({ gemini: geminiKey, pexels: pexelsKey });
  };

  const shouldShowGemini = missingKeys.includes('GEMINI');
  const shouldShowPexels = missingKeys.includes('PEXELS');

  const isSaveDisabled = 
    (shouldShowGemini && !geminiKey) ||
    (shouldShowPexels && !pexelsKey);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-[100]">
      <div className="bg-slate-800 rounded-lg shadow-2xl p-8 w-full max-w-md m-4">
        ...
        <p className="text-gray-400 mb-6 text-sm">
          Please provide your API key(s) to use the application. This key is only stored in your browser for this session and will be deleted when you close this tab.
        </p>
